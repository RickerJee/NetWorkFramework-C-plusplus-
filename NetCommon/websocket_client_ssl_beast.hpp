...
void do_session(std::string host,std::string const& port,std::string const& text,net::io_context& ioc,ssl::context& ctx,net::yield_context yield)
{
	beast::error_code ec;
...
int websocket_client_ssl_init(const std::string& host, const std::string& port, const std::string& text)
{
	//if (argc != 4)
	//{
	//	std::cerr <<
